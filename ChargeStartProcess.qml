...
Rectangle {
    ...
    Timer {
        interval: 100; running: true; repeat: true
        onTriggered: {
            //console.log("IsRemoteStart : ",wallboxCharger.isRemoteStart)
            if(wallboxCharger.isRemoteStart === 1)
            {
    ...
    Text {
        ...
        height: 40
        color: "#ffffff"
        text: qsTr("Şarjı başlatmak için:")
        font.pixelSize: 22
        horizontalAlignment: Text.AlignHCenter
        verticalAlignment: Text.AlignVCenter
        font.family: customFont.name
